...
public class ProductService {
    ...
    public Page<ProductDTO> getProductsPaginated(int page, int size, String sort, Integer colorId, Integer materialId, Integer maxPrice, String keyword) {
        ...
        if (null != sort) // Kiểm tra chuỗi 'sort' truyền từ frontend để đổi cách sắp xếp
        switch (sort) {
            case "price-asc" -> sortOrder = Sort.by("price").ascending();   // Giá tăng dần
            case "price-desc" -> sortOrder = Sort.by("price").descending();  // Giá giảm dần
            case "rating" -> // Vì averageRating là trường @Formula nên Hibernate tự hiểu và sort trong SQL được luôn!
                sortOrder = Sort.by("averageRating").descending(); // Đánh giá cao nhất
            case "name-asc" -> sortOrder = Sort.by("name").ascending();    // Tên từ A đến Z
            case "name-desc" -> sortOrder = Sort.by("name").descending();   // Tên từ Z đến A
            default -> {
            }
        }
